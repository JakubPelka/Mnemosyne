...
class UnitBuilder:
    def __init__(self, max_chars=18000, max_events=10, overlap_events=1, strategy_version="unit-v1"):
        self.max_chars = max_chars
        self.max_events = max_events
        self.overlap_events = overlap_events
        self.strategy_version = strategy_version

    def build_units_for_context(self, context_id: str, events: List[Dict[str, Any]], title: str = "") -> List[Dict[str, Any]]:
        # Sort events by time
        ...
        def commit_unit(unit_events, seq_no):
            if not unit_events: return None
            ...
            content_hash_raw = f"{title}|{ordered_ids_str}|{normalized_content}|{self.strategy_version}"
        ...
        while idx < len(events):
            ...
            if len(current_unit) >= self.max_events or (current_chars + txt_len > self.max_chars and len(current_unit) > 0):
                # Commit current
