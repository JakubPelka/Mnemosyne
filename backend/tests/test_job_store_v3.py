...
def test_attempt_history_is_append_only(store):
    ...
    # Re-claim job requires retry
    store.retry_job(job["job_id"], "test retry")
    ...
    with sqlite3.connect(store.db_path) as conn:
        ...
        attempts = conn.execute(
            "SELECT * FROM tagging_attempt WHERE job_id = ? ORDER BY attempt_no", (job["job_id"],)
        ).fetchall()
...
def test_complete_fail_with_token(store):
    ...
    # Correct token works
    store.complete_job(
        job["job_id"], job["attempt_id"], job["lease_token"], "{}", "out", 100, 10, 10
    )
