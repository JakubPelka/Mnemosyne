...
class DummyStore(JobStore):
    ...
    def fail_job(self, job_id, attempt_id, lease_token, error_code, error_summary, done_reason=None):
        self.failed_jobs.append((job_id, error_code, error_summary, done_reason))

    def complete_job(self, job_id, attempt_id, lease_token, output_json, output_hash, elapsed_ms, p_tok, c_tok):
        self.completed_jobs.append((job_id, output_json, p_tok, c_tok))

    def record_attempt_response_metadata(self, attempt_id, job_id, lease_token, elapsed_ms, p_tok, c_tok, done_reason):
        self.metadata.append((attempt_id, job_id, lease_token, elapsed_ms, p_tok, c_tok, done_reason))
...
def test_done_reason_length_is_truncated():
    ...
    job = {"job_id": "j1", "attempt_count": 1, "attempt_id": "a1", "lease_token": "l1", "num_predict": 4096, "prompt_version": "semantic-hybrid-v2", "schema_version": "semantic-tags-v2", "settings": {"num_predict": 4096, "seed": 42, "num_ctx": 8192}, "seed": 42, "num_ctx": 8192}
...
def test_num_ctx_in_hash(tmp_path):
    ...
    run_info = {"model_name": "m", "prompt_version": "p", "schema_version": "s", "unit_strategy_version": "u", "settings": {"num_ctx": 8192}}
